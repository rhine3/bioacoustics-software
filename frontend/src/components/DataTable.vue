...
<template>
  ...
  <div class="max-h-32 overflow-y-scroll mb-5">
    <div class="flex flex-wrap place-content-center">
      ...
      >
        <input
          v-bind:id="key"
          v-bind:checked="visibleKeys.includes(key)"
          @change="updateChecked(key)"
          type="checkbox"
          v-bind:value="key"
          v-bind:name="key"
  ...
  <div class="data-table-wrapper overflow-x-auto overflow-y-auto" style="height: 550px;">
    <DataModal ref="modalRef" v-bind:data="currentData" />
      ...
      <thead class="text-xs text-gray-700 uppercase bg-gray-50">
        <tr>
          <th
            ...
            scope="col"
            class="px-6 py-3 whitespace-nowrap text-ellipsis select-none cursor-pointer"
          >
            {{ nameLookup[key] }}
            <span>
              <svg
                v-if="key === sortBy.key"
                class="w-2 h-2 mb-1 inline-block text-gray-800 dark:text-white"
                aria-hidden="true"
      ...
      <tbody>
        ...
        >
          <td
            v-for="key in visibleKeys"
            scope="row"
            class="px-6 py-4 font-medium text-gray-900 align-top break-words max-w-xs"
          >
